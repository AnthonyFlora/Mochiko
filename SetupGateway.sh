...
sudo bash -c 'echo "bridge=br0" >> /etc/hostapd/hostapd.conf'
...
echo '\n*** UPDATING BRIDGE ***'
echo 'y' | sudo apt-get install bridge-utils
sudo brctl addbr br0
sudo brctl addif br0 wlan1
sudo bash -c 'echo "auto br0" >> /etc/network/interfaces'
sudo bash -c 'echo "iface br0 inet manual" >> /etc/network/interfaces'
sudo bash -c 'echo "bridge_ports wlan1 wlan0" >> /etc/network/interfaces'
